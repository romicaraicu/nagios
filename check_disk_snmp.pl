...
###--------------------------------------------------------------------------###
#print "unit_sz=$unit_sz \n";

my $var = $resp->{$devSize}/100*95;

$unit_sz	= $unit_sz / $resp->{$devUnit};
$perc		= int(($resp->{$devUsed} / $resp->{$devSize}) * 100 + 5);
#$perc		= int(($resp->{$devUsed} / $var) * 100);

#print "devUsed=$resp->{$devUsed} \n";
#print "devSize=$resp->{$devSize} \n";
#print "perc=$perc \n";
#print "unit_size=$unit_sz \n";
...
$size		= sprintf("%0.2f", $var / $unit_sz);
$free		= sprintf("%0.2f",
  ($var - $resp->{$devUsed}) / $unit_sz
);
